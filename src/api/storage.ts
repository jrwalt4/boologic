...
import {
  defaultRegistry,
  getOperation as getOperationSync,
  ID,
  OperationDefinition,
  OperationRegistry
} from "./operation";
...
export async function getOperation(opId: ID): Promise<OperationDefinition> {
  return getOperationSync(await _getOperations(), opId);
}
